...
_main:
	...
	add	a,#0x0a
	mov	sp,a
;	src/main.c:26: uart.Baud       = 12;
	mov	a,_bp
	inc	a
	mov	r0,a
	mov	@r0,#0x0c
;	src/main.c:27: uart.Parity     = UART_NO_PARITY;
	mov	r7,_bp
	inc	r7
	mov	a,r7
	inc	a
	mov	r0,a
	mov	@r0,#0x00
;	src/main.c:28: uart.DataLength = UART_DATA_LEN_8BIT;
	mov	a,#0x02
	add	a,r7
	mov	r0,a
;	src/main.c:29: uart.StopBits   = UART_1_STOP_BIT;
	mov	a,#0x03
	mov	@r0,a
	add	a,r7
	mov	r0,a
...
;	src/main.c:36: cli();
	anl	_IE,#0x7f
;	src/main.c:38: SysClock_Config();
	push	ar7
	lcall	_SysClock_Config
	pop	ar7
;	src/main.c:39: UART_Config(&uart);
	mov	r6,#0x00
	mov	r5,#0x40
	mov	dpl,r7
	mov	dph,r6
...
;	src/main.c:40: I2C_Config(&i2c);
	...
	mov	b,r5
	lcall	_I2C_Config
;	src/main.c:41: GPIO_Config();
	lcall	_GPIO_Config
;	src/main.c:42: SysTick_Init();
	lcall	_SysTick_Init
;	src/main.c:45: sei();
	mov	r6,_IE
	orl	ar6,#0x80
	mov	_IE,r6
;	src/main.c:46: GPIOPin_Write(GPIO_LED2, GPIO_SET);
	mov	a,#0x01
	push	acc
	mov	dpl,#0x01
	lcall	_GPIOPin_Write
	dec	sp
;	src/main.c:47: GPIOPin_Write(GPIO_COM_MODE, GPIO_RESET);
	clr	a
	push	acc
	mov	dpl,#0x05
	lcall	_GPIOPin_Write
	dec	sp
;	src/main.c:48: GPIOPin_Write(GPIO_DCDC_EN, GPIO_SET);
	mov	a,#0x01
	push	acc
	mov	dpl,#0x04
	lcall	_GPIOPin_Write
	dec	sp
;	src/main.c:49: GPIOPin_Write(GPIO_5V_CNTL, GPIO_SET);
	mov	a,#0x01
	push	acc
	mov	dpl,#0x03
	lcall	_GPIOPin_Write
...
00108$:
;	src/main.c:64: Delayms(2000);
	mov	dptr,#0x07d0
	lcall	_Delayms
;	src/main.c:65: UARTCR2 = (UARTCR2 & (~UARTCR2_RXE));
	anl	_UARTCR2,#0xfb
;	src/main.c:75: UARTCR2 = ((UARTCR2 & (~UARTCR2_RXE)) | UARTCR2_RXE);
	mov	a,#0xfb
	anl	a,_UARTCR2
	orl	a,#0x04
	mov	_UARTCR2,a
;	src/main.c:89: GPIOPin_Toggle(GPIO_LED2);
	mov	dpl,#0x01
